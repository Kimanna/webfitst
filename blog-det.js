...
// 댓글을 펼치는 클릭리스너와, 댓글의 수정삭제 로직
 function inputCommentArea (user_id) {
  ...
  // 삭제 클릭시 alart 생성
  $('.comment_delete_bt').on('click', function(){
    $(this).siblings('.cm_replybox').removeClass('hidden');
                            ...
                            <div class="cm_content_area2" style="width:100%">
                                ...
                                <div class="cm_replace_area2">
                                  ...
                                  <div id="${comments_comment[temp].comment_no}" class="cm_replybt">
                                    ...
                                    <span class="comments_like" style="margin-left:10px;"></span>
                                    ...
                                    <span class="comments_dislike" style="margin-left:10px;"></span>
...
function commentsCommentLike (login_user_id) {
 ...
 function inputCommentsCommentArea () {
   ...
   $('.show_comment_comment_area').on('click', function(){
    ...
    // 삭제 클릭시 alart 생성
    $('.comments_comment_delete_bt').on('click', function(){
      $(this).siblings('.cm_replybox').removeClass('hidden');
...
function update_comment(comment_no, action) {
  ...
  var user_id = $('.login_user_id').attr('name'); // 현재 로그인 중인 user 의 id -> 대댓글 입력하는 html에 들어가는 부분 
  var revised_comment = $('#commentId_'+comment_no).nextAll('.cm_reply_update').children('.send_comment_text').val();
  ...
  if (action == 'update') {
        ...
        $.ajax({
          ...
          dataType:"json",
          url:"comment.php",
          data:{'mode':'comment_delete', 'comment_no':comment_no},
          success : function (json_data) {
            console.log(json_data);
    ...
    if (user_id == "" || user_id == undefined) {
      ...
      return;
        ...
        $.ajax({
          ...
          dataType:"json",
          url:"comment.php",
          data:{'mode':'comments_comment_delete', 'comment_no':comment_no},
          success : function (json_data) {
            console.log(json_data);
